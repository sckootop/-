...
void USSaveGameSubsystem::WriteSaveGame()
{
	...
	USSaveGame* Temp_SaveGame = Cast<USSaveGame>(UGameplayStatics::LoadGameFromSlot("SaveGameToLoad", 0));
	if (CurrentSlotName == "NewSlot") {
		if (Temp_SaveGame == nullptr)
		{
	...
	else if (CurrentSlotName == "NewSlot") {
		Temp_SaveGame->SavedPlayers.Empty();
		Temp_SaveGame->SavedActors.Empty();
	}
	...
	for (int32 i = 0; i < GS->PlayerArray.Num(); i++)
	{
		...
		{	
			if (CurrentSlotName == "NewSlot") {
				PS->SavePlayerState(Temp_SaveGame);
			}
			else {
	...
	for (FActorIterator It(GetWorld()); It; ++It)
	{
		...
		if (CurrentSlotName == "NewSlot") {
			Temp_SaveGame->SavedActors.Add(ActorData);
		}
		else {
	...
	if (CurrentSlotName == "NewSlot") {
		CurrentSaveGame = Temp_SaveGame;
		CurrentSlotName = "SaveGameToLoad";
	}
	...
	if (UGameplayStatics::DoesSaveGameExist(CurrentSlotName, 0))
	{
		...
		for (FActorIterator It(GetWorld()); It; ++It)
		{
			...
			for (FActorSaveData ActorData : CurrentSaveGame->SavedActors)
			{
				...
				{
					UE_LOG(LogTemp, Log, TEXT("gain actor information %s"), *Actor->GetName());
...
void USSaveGameSubsystem::CreateNewGame()
{
	SetSlotName("NewSlot");
	CurrentSaveGame = Cast<USSaveGame>(UGameplayStatics::CreateSaveGameObject(USSaveGame::StaticClass())); 
	UGameplayStatics::SaveGameToSlot(CurrentSaveGame, CurrentSlotName, 0);
	UE_LOG(LogTemp, Log, TEXT("Created NewGame Data."));
